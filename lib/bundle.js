...
var Panda = function () {
  ...
  _createClass(Panda, [{
    ...
    value: function blockBounce(block) {
      // console.log(block.posX);
      // this.panda.x += this.dx;
      // this.panda.y += this.dy;
      // let disX = Math.abs(this.panda.x - block.block.x);
      // let disY = Math.abs(this.panda.y - block.block.y);
      if (this.panda.x > block.posX && this.panda.x < block.posX + 50 && this.panda.y > block.posY && this.panda.y < block.posY + 20) {
        block.health -= 1;
        this.dx = -this.dx;
        this.dy = -this.dy;
        if (block.health <= 0) {
