      ...
      if (e.keyCode === 13 && !this.isPlaying) {
        ...
        this.ticker.addEventListener("tick", function () {
          ...
          _this2.blocks.forEach(function (block, idx) {
            _this2.pandas.forEach(function (panda) {
              panda.blockBounce(block);
            });
            if (block.health <= 0) {
              if (block.health <= 0 && block.powerUp === 1) {
                var panda1 = new _panda2.default(_this2);
                var panda2 = new _panda2.default(_this2);
              ...
              if (block.health <= 0 && block.powerUp === 2) {
                var barrier = new _blocks2.default(_this2, 1, 0, 565);
                barrier.barrier();
              }
          ...
          _this2.pandas.forEach(function (panda, idx) {
            panda.bounce(_this2.bamboo);
          });
          _this2.stage.update();
          for (var i = 0; i < _this2.pandas.length; i++) {
            if (_this2.pandas[i].panda.y > 585) {
              _this2.stage.removeChild(_this2.pandas[i].panda);
              _this2.pandas.splice(i, 1);
            }
          }
          if (_this2.pandas.length === 0) {
...
var Block = function () {
  ...
  _createClass(Block, [{
    ...
    value: function initiate(posX, posY, width, height, powerUp) {
      if (this.powerUp === 1) {
        this.block.graphics.beginStroke("#000").beginFill("red").drawRect(posX, posY, width, height);
      } else if (this.powerUp === 2) {
        this.block.graphics.beginStroke("#000").beginFill("blue").drawRect(posX, posY, width, height);
